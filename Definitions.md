...
- Saga Effects - ...
----------------
- **Web Animations API** — это стандарт, который дает гибкость JS, сохраняя производительность CSS-анимации
...
- Препроцессор CSS - 
- Постпроцессор CSS - 
- **LESS** - динамический язык стилей, обеспечивает следующие расширения CSS: переменные, вложенные блоки, миксины, операторы и функции[2].
- **SASS (Syntactically Awesome Style Sheets)** - это метаязык на основе CSS, предназначенный для увеличения уровня абстракции CSS-кода и упрощения файлов каскадных таблиц стилей.
- **SCSS (short for syntactically awesome style sheets)** - "диалект" языка SASS. А что такое SASS? SASS это язык похожий на HAML (весьма лаконичный шаблонизатор), но предназначенный для упрощения создания CSS-кода
- Stylus
- PostCSS
----------------
...
- **The OpenAPI Specification** — формализованная спецификация и экосистема множества инструментов, предоставляющая интерфейс между front-end системами, кодом библиотек низкого уровня и коммерческими решениями в виде API. Вместе с тем, cпецификация построена таким образом, что не зависит от языков программирования, и удобна в использовании как человеком, так и машиной
----------------
...
- Структура данных - ...
- Priority Queue - ...
- 2 типа разрешений коллизий: "раздельные цепочки" и "открытая адресация"
- Граф - ...
...
- Таблицу, которую партицируют, называют `мастер-таблицей`.
- **Партицирование** решает следующие проблемы:
1) **ускорение выборки** данных;
2) **ускорение вставки** данных;
3) **упрощение удаления старых** данных;
4) упрощение обслуживания таблицы.
----------------
- NoSQL - ...
----------------
...
----------------
- **Data Expiration Types and Strategies:**
- Absolute Expiration
- Sliding Expiration
----------------
